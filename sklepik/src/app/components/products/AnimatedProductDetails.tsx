...
export default function AnimatedProductDetails({ product}: AnimatedProductDetailsProps) {
        ...
        >
          ...
          >
            <motion.button  onClick={() => handleAddToCart(product)}
              className={"w-full bg-blue-600  text-white py-4 uppercase tracking-wider text-sm font-medium transition-all duration-300" + (product.stock<=0 ? " cursor-not-allowed opacity-50 disabled" : "hover:bg-blue-700")}
              whileTap={{ scale: 0.98 }}
            >
              Dodaj do koszyka
            </motion.button>
            {product.stock <= 0 && (<p className="mt-4 text-red-400 uppercase font-body">Obecnie niedostępny</p>)}
          </motion.div>
        </motion.div>
      </div>
    </div>
